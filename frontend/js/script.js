...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const generalDashboardContent = document.getElementById('dashboard-content');
    ...
    const parentAnnotationsSection = document.getElementById('parent-annotations-section');
    const parentAnnotationsContent = document.getElementById('parent-annotations-content');
    const noParentAnnotationsMessage = parentAnnotationsContent?.querySelector('.no-annotations-message');
    const parentChartsGrid = document.getElementById('parent-charts-grid');
    ...
    const printTherapistNotesSection = document.getElementById('print-therapist-notes-section');
    const printTherapistNotesContent = document.getElementById('print-therapist-notes-content');
    ...
    function initializeApp() {
        console.log("ABAplay v3.9.5 (Relatório Formatado) - Inicializando...");
    ...
    async function startAuthenticatedSession() {
        if (!isAuthenticated || !currentUser) { showLoginScreen(); return; }
        showMainAppScreen();
    ...
    function hideTherapistFeatures() {
        ...
        topNavLinks.forEach(link => {
            ...
            if (viewsToHideForParents.includes(view)) link.classList.add('hidden');
    ...
    async function loadParentDashboardData() {
        ...
        if (parentDashboardContent && parentDashboardContent !== parentLoadingPlaceholder) parentDashboardContent.innerHTML = '';
        if (parentAnnotationsSection) parentAnnotationsSection.classList.add('hidden');
        if (parentChartsGrid) parentChartsGrid.innerHTML = '';
        ...
        try {
            ...
            if (response.ok) {
                ...
                } else { throw new Error("Dados do paciente não retornados pela API."); }
    ...
    function renderParentDashboard(patientData, sessionDataArray, assignedProgramIdsArray) {
        ...
        if (parentAnnotationsSection && parentAnnotationsContent) {
            parentAnnotationsSection.classList.remove('hidden');
            ...
            parentDashboardContent.appendChild(parentAnnotationsSection);
        }

        const chartsGridContainer = document.createElement('div');
        chartsGridContainer.id = 'parent-charts-grid';
        chartsGridContainer.className = 'grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 mt-6';
    ...
    function renderSingleConsolidatedChartForParent(allPatientSessions, programId, canvasId) {
        ...
        if (!canvas || !canvasContainer || !noDataMsg) return;
        ...
        if (consolidatedChartInstances[canvasId]) consolidatedChartInstances[canvasId].destroy();
    ...
    function setupMainEventListeners() {
        ...
        if (currentUser && currentUser.role === 'terapeuta') {
            if (clientSearchInput) clientSearchInput.addEventListener('input', handleClientSearch);
            if (showAddClientFormBtn) showAddClientFormBtn.addEventListener('click', handleShowAddClientForm);
            if (closeAddClientPanelBtn) closeAddClientPanelBtn.addEventListener('click', handleCloseAddClientPanel);
            if (triggerAddClientFormBtn) triggerAddClientFormBtn.addEventListener('click', handleTriggerAddClientForm);
            if (clientListUl) clientListUl.addEventListener('click', handleClientListClick);
            if (addClientForm) addClientForm.addEventListener('submit', handleAddOrEditClientSubmit);
            if (editClientBtn) editClientBtn.addEventListener('click', handleEditClient);
            if (deleteClientBtn) deleteClientBtn.addEventListener('click', handleDeleteClient);
            if (generateClientGradePdfBtn) generateClientGradePdfBtn.addEventListener('click', handleGenerateGradePdf);
            if (generateClientRecordPdfBtn) generateClientRecordPdfBtn.addEventListener('click', handleGenerateRecordPdf);
            if (generateConsolidatedReportBtn) generateConsolidatedReportBtn.addEventListener('click', openConsolidatedReportModal);
            if (programSearchInput) programSearchInput.addEventListener('input', handleProgramSearch);
    ...
    function handleNavClick(e) {
        ...
        if (currentUser.role === 'pai') {
            if (effectiveViewId !== 'parent-dashboard-view') effectiveViewId = 'parent-dashboard-view';
            areaToSwitch = null;
        } else if (currentUser.role === 'terapeuta') {
            if (effectiveViewId === 'parent-dashboard-view') effectiveViewId = 'dashboard-view';
        }
        if (effectiveViewId) {
            if (areaToSwitch && currentUser.role === 'terapeuta') currentActiveArea = areaToSwitch;
    ...
    function handleClientListClick(e) { if (isAuthenticated && currentUser.role === 'terapeuta') { const clientItem = e.target.closest('.client-item'); if (clientItem && clientItem.dataset.clientId) { const patientToSelect = patients.find(p => String(p.id) === clientItem.dataset.clientId); if (patientToSelect) { selectPatient(patientToSelect); if (window.innerWidth < 768 && contextualSidebar) contextualSidebar.classList.add('hidden'); } } } }
    ...
    function handleProgramSearch(e) { if (isAuthenticated && currentUser.role === 'terapeuta') { const activeProgramLink = programCategoriesContainer.querySelector('a.program-link.active'); filterAndDisplayPrograms(currentActiveArea, activeProgramLink?.dataset.programId, e.target.value); } }
    ...
    function resetUIForLogout() {
        ...
        currentClientIndicator.classList.remove('font-medium', 'text-indigo-700', 'hidden');
        ...
        updatePatientCountDisplay();
        if(patientCountIndicator) patientCountIndicator.classList.add('hidden');
        if (notesViewTextarea) { notesViewTextarea.value = ''; notesViewTextarea.disabled = true; }
        if (saveNotesBtn) saveNotesBtn.disabled = true;
        if (notesPlaceholder) notesPlaceholder.classList.remove('hidden');
        if (generalDashboardContent) {
            generalDashboardContent.classList.remove('hidden');
        ...
        if (clientDashboardContent) { clientDashboardContent.classList.add('hidden'); clientDashboardContent.innerHTML = ''; }
        if (parentDashboardView) parentDashboardView.classList.add('hidden');
        if (parentDashboardContent) parentDashboardContent.innerHTML = '';
        if (parentLoadingPlaceholder && parentDashboardContent) parentDashboardContent.appendChild(parentLoadingPlaceholder);
        if (parentAnnotationsSection) parentAnnotationsSection.classList.add('hidden');
        if(showAddClientFormBtn) { showAddClientFormBtn.disabled = true; showAddClientFormBtn.title = ''; showAddClientFormBtn.classList.remove('hidden'); }
        views.forEach(view => view.classList.add('hidden'));
        topNavLinks.forEach(link => link.classList.remove('active', 'hidden'));
        mobileNavLinks.forEach(link => link.classList.remove('active', 'hidden'));
        ...
        if (contextualSidebar) {
            ...
            if (window.innerWidth < 768) contextualSidebar.classList.add('hidden');
        }
    ...
    function updatePatientCountDisplay() {
        if (isAuthenticated && currentUser && currentUser.role === 'terapeuta') {
            ...
            if(patientCountIndicator) patientCountIndicator.classList.add('hidden');
            if(showAddClientFormBtn) { showAddClientFormBtn.disabled = true; showAddClientFormBtn.title = ''; }
        }
    }
    ...
    async function loadProgramsData() {
        try {
            ...
            if (!allProgramsData[currentActiveArea] && currentUser?.role === 'terapeuta') {
                ...
                if (firstArea) currentActiveArea = firstArea;
                else allProgramsData = {};
            }
        } catch (error) { allProgramsData = {}; }
    }
    ...
    async function loadUserPatientsFromAPI() {
        if (!isAuthenticated || !authToken || currentUser?.role !== 'terapeuta') {
            patients = [];
            if (currentUser?.role === 'terapeuta') { renderClientList(); updatePatientCountDisplay(); }
            return;
        }
        try {
    ...
    function switchView(viewId, context = {}) {
        ...
        if (currentUser.role === 'pai') {
            ...
            if (effectiveViewId === 'parent-dashboard-view') effectiveViewId = 'dashboard-view';
            if (effectiveViewId === 'programs-view' && areaFromContext) currentActiveArea = areaFromContext;
        }
        currentView = effectiveViewId;
        topNavLinks.forEach(link => {
            const linkView = link.dataset.view, linkArea = link.dataset.area;
            let isActive = linkView === currentView;
            if (linkView === 'programs-view' && currentUser.role === 'terapeuta') isActive = isActive && linkArea === currentActiveArea;
            else if (linkView === 'parent-dashboard-view' && currentUser.role === 'pai' && currentView === 'parent-dashboard-view') isActive = true;
        ...
        if (parentDashboardView) parentDashboardView.classList.toggle('hidden', currentView !== 'parent-dashboard-view');
        updateContextualSidebar(currentView, currentActiveArea);
        ...
        switch (currentView) {
            ...
            case 'dashboard-view': if (currentUser.role === 'terapeuta') renderDashboard(); break;
    ...
    function updateContextualSidebar(viewId, activeArea) {
        ...
        if (contextualSidebar) {
            if (window.innerWidth < 768) {
                ...
                if (isMobileMenuOpen || isAddClientPanelVisible) contextualSidebar.classList.add('hidden');
                else contextualSidebar.classList.remove('hidden');
            } else {
                contextualSidebar.classList.remove('hidden');
                contextualSidebar.classList.add('md:block');
            }
    ...
    function handleClientViewSwitch(context) {
        ...
        if (!(context?.showAddPanel) && addClientPanel) addClientPanel.classList.add('hidden');
        ...
        if(programCategoriesContainer) {
            ...
            if(programId) {
                ...
                if(link) {
                    ...
                    if(details) { if(!details.open) details.open = true; details.querySelector('summary')?.classList.add('active'); }
                }
            }
        }
    ...
    function handleNotesViewSwitch() {
        if (!isAuthenticated || currentUser.role !== 'terapeuta') return;
        if (selectedPatient) {
            if(notesViewTextarea) { notesViewTextarea.value = selectedPatient.general_notes || ''; notesViewTextarea.disabled = false; }
    ...
    function displayNoClientSelected() {
        ...
        if(noClientSelectedPlaceholder) noClientSelectedPlaceholder.classList.remove('hidden');
        if(addClientPanel) addClientPanel.classList.add('hidden');
        if(assignedProgramsListUl) { assignedProgramsListUl.innerHTML = ''; if(noAssignedProgramsLi) { noAssignedProgramsLi.classList.remove('hidden'); assignedProgramsListUl.appendChild(noAssignedProgramsLi); } }
        if(progressDetailsAreaDiv) progressDetailsAreaDiv.innerHTML = `<div class="text-center text-gray-500 py-6"><i class="fas fa-tasks text-3xl mb-3 text-gray-400"></i><p class="text-sm">Selecione um programa atribuído na lista ao lado para registrar uma sessão ou ver o gráfico de progresso.</p></div>`;
        if (currentView === 'programs-view') filterAndDisplayPrograms(currentActiveArea, null, programSearchInput ? programSearchInput.value : '');
        else if (currentView === 'notes-view') handleNotesViewSwitch();
        else if (currentView === 'dashboard-view') renderDashboard();
    }
    function displayClientDetails(patient) {
        ...
        if(progressDetailsAreaDiv) progressDetailsAreaDiv.innerHTML = `<div class="text-center text-gray-500 py-6"><i class="fas fa-tasks text-3xl mb-3 text-gray-400"></i><p class="text-sm">Selecione um programa atribuído na lista ao lado para registrar uma sessão ou ver o progresso.</p></div>`;
        selectedProgramForProgress = null;
    }
    async function handleAddOrEditClientSubmit(e) {
        ...
        let url = `${API_BASE_URL}/patients`, method = 'POST';
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (foundPatient) selectPatient(foundPatient);
                else displayNoClientSelected();
                updateContextualSidebar(currentView, currentActiveArea);
            } else { alert(`Erro: ${result.errors?.[0]?.msg || 'Falha ao salvar.'}`); }
    ...
    async function deleteClient() {
        ...
        if (confirm(`Tem certeza que deseja excluir o cliente "${selectedPatient.name}"? Esta ação não pode ser desfeita.`)) {
            try {
                const response = await fetch(`${API_BASE_URL}/patients/${selectedPatient.id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${authToken}` } });
                if (response.ok || response.status === 204) {
                    alert(`Cliente "${selectedPatient.name}" excluído com sucesso.`);
                    displayNoClientSelected();
                    await loadUserPatientsFromAPI();
                } else { const result = await response.json().catch(() => ({ errors: [{ msg: response.statusText }] })); alert(`Erro ao excluir: ${result.errors?.[0]?.msg || 'Falha.'}`); }
            } catch (error) { alert("Erro de conexão."); }
        }
    }
    function editClient() {
        ...
        if (selectedPatient.dob) {
            ...
            } catch (e) { clientDobInput.value = selectedPatient.dob.split('T')[0] || ''; }
        } else { clientDobInput.value = ''; }
    ...
    function renderProgramCategories(activeArea) {
        ...
        programCategoriesContainer.querySelectorAll('a.program-link').forEach(link => { link.addEventListener('click', handleProgramLinkClick); });
        programCategoriesContainer.querySelectorAll('details.program-category-details').forEach(detailsElement => { detailsElement.addEventListener('toggle', handleCategoryToggle); });
    }
    ...
    function createProgramCardElement(program, currentSelectedPatient) {
        ...
        const procedurePreviewHTML = program.procedure?.slice(0, 2).map(step => `<p class="text-xs text-gray-600 mb-1 truncate" title="${step.term}: ${step.description}"><strong class="font-medium text-gray-800">${step.term}:</strong> ${step.description}</p>`).join('') + (program.procedure?.length > 2 ? '<p class="text-xs text-gray-400 italic mt-1">...</p>' : '') || '<p class="text-xs text-gray-400 italic">Procedimento não detalhado.</p>';
        const materialsHTML = program.materials?.length > 0 ? `<div class="mt-2"><h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Materiais</h4><ul class="list-disc list-inside text-xs text-gray-600 space-y-0.5">${program.materials.slice(0, 3).map(item => `<li class="truncate" title="${item}">${item}</li>`).join('')}${program.materials.length > 3 ? '<li class="text-gray-400 italic">...</li>' : ''}</ul></div>` : '';
        let assignButtonHTML = '';
        if (currentUser.role === 'terapeuta') {
            const isAssigned = currentSelectedPatient?.assigned_program_ids?.includes(program.id);
            assignButtonHTML = currentSelectedPatient ? `<button class="assign-program-btn text-xs font-medium py-1.5 px-3 rounded ${isAssigned ? 'assigned bg-emerald-500 text-white cursor-default' : 'bg-indigo-100 text-indigo-700 hover:bg-indigo-200'}" data-program-id="${program.id}" title="${isAssigned ? 'Programa Já Atribuído' : 'Atribuir ao Cliente'}" ${isAssigned ? 'disabled' : ''}><i class="fas ${isAssigned ? 'fa-check' : 'fa-plus'} mr-1"></i> ${isAssigned ? 'Atribuído' : 'Atribuir'}</button>` : `<button class="text-xs font-medium py-1.5 px-3 rounded bg-gray-100 text-gray-500 cursor-not-allowed" disabled title="Selecione um cliente para atribuir programas"><i class="fas fa-plus mr-1"></i> Atribuir</button>`;
        }
        card.innerHTML = `<div><div class="flex justify-between items-start mb-2"><h3 class="text-base font-semibold text-gray-800 leading-tight">${program.title}</h3><span class="tag text-xs font-semibold uppercase px-2 py-0.5 rounded-full ${getTagColor(program.tag || 'N/A')} flex-shrink-0 ml-2">${program.tag || 'N/A'}</span></div><p class="text-xs font-medium text-indigo-600 mb-1">Objetivo:</p><p class="text-xs text-gray-600 mb-3 line-clamp-2" title="${program.objective || ''}">${program.objective || 'Não definido'}</p>${materialsHTML}<div class="mt-3"><h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Procedimento (Prévia)</h4>${procedurePreviewHTML}</div></div>${currentUser.role === 'terapeuta' ? `<div class="program-card-actions mt-4 pt-3 border-t border-gray-100 text-right">${assignButtonHTML}</div>` : ''}`;
        if (currentUser.role === 'terapeuta') { const assignButton = card.querySelector('.assign-program-btn'); if (assignButton && !assignButton.disabled) assignButton.addEventListener('click', handleAssignProgram); }
        return card;
    }
    async function handleAssignProgram(e) {
        ...
        if (!program) { alert("Erro: Programa não encontrado."); return; }
        if (selectedPatient.assigned_program_ids?.includes(programId)) return;
        button.disabled = true; button.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i> Atribuindo...';
        try {
            ...
            } else { const result = await response.json().catch(() => ({ errors: [{ msg: "Erro ao processar resposta do servidor." }] })); alert(`Erro ao atribuir programa: ${result.errors?.[0]?.msg || 'Falha na operação.'}`); button.disabled = false; button.innerHTML = `<i class="fas fa-plus mr-1"></i> Atribuir`; }
        } catch (error) { alert("Erro de conexão ao tentar atribuir o programa."); button.disabled = false; button.innerHTML = `<i class="fas fa-plus mr-1"></i> Atribuir`; }
    }
    async function handleRemoveProgram(e) {
        ...
        if (confirm(`Tem certeza que deseja remover o programa "${programTitle}" de ${selectedPatient.name}?`)) {
            ...
            try {
                ...
                } else { const result = await response.json().catch(() => ({ errors: [{ msg: "Erro ao processar resposta do servidor." }] })); alert(`Erro ao remover programa: ${result.errors?.[0]?.msg || 'Falha na operação.'}`); button.disabled = false; }
            } catch (error) { alert("Erro de conexão ao tentar remover o programa."); button.disabled = false; }
        }
    }
            ...
            assignedProgramsDetails.forEach(program => {
                ...
                li.innerHTML = `
                    ...
                    <div class="program-actions flex space-x-1 sm:space-x-2 flex-shrink-0 no-print">
                        <button class="view-progress-btn text-xs font-medium text-indigo-600 hover:text-indigo-800 px-1.5 py-1 sm:px-2 rounded hover:bg-indigo-50 flex items-center" title="Ver Progresso/Registrar Sessão" data-program-id="${program.id}"><i class="fas fa-chart-line fa-fw mr-1"></i> <span class="hidden sm:inline">Ver/Reg.</span></button>
                        <button class="remove-program-btn text-xs font-medium text-red-500 hover:text-red-700 px-1.5 py-1 sm:px-2 rounded hover:bg-red-50 flex items-center" title="Remover Programa" data-program-id="${program.id}"><i class="fas fa-times-circle fa-fw mr-1"></i> <span class="hidden sm:inline">Remover</span></button>
                    </div>`;
    ...
    function handleViewProgramProgress(e) {
        ...
        if (selectedProgramForProgress) {
            ...
            if(assignedProgramsListUl) assignedProgramsListUl.querySelectorAll('.assigned-program-item').forEach(item => item.classList.toggle('active', item.dataset.programId === programId));
        } else { clearSessionProgressArea(); }
    }
    function renderSessionEntryForm(program) {
        ...
        progressDetailsAreaDiv.innerHTML = `
            <h4 class="text-base font-semibold text-gray-700 mb-3">Registrar Sessão: <span class="font-normal">${program.title}</span></h4>${criteriaHTML}
            <form id="add-session-form" data-program-id="${program.id}" class="space-y-3 mb-6 pb-6 border-b border-gray-200">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div><label for="session-date" class="block text-xs font-medium text-gray-600 mb-1">Data:</label><input type="date" id="session-date" required value="${new Date().toISOString().split('T')[0]}" class="w-full px-2 py-1.5 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 text-sm shadow-sm"></div>
                    <div><label for="session-correct-trials" class="block text-xs font-medium text-gray-600 mb-1">Nº de Acertos Realizados:</label><input type="number" id="session-correct-trials" min="0" ${totalTrials > 0 ? `max="${totalTrials}"` : ''} step="1" required placeholder="Nº de acertos" class="w-full px-2 py-1.5 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 text-sm shadow-sm">${totalTrials > 0 ? `<p class="text-xs text-gray-500 mt-1">De ${totalTrials} tentativas planejadas.</p>` : '<p class="text-xs text-red-500 mt-1">Número de tentativas não definido para este programa.</p>'}</div>
                </div>
                <div><label for="session-notes" class="block text-xs font-medium text-gray-600 mb-1">Observações:</label><textarea id="session-notes" rows="2" placeholder="Alguma observação sobre a sessão?" class="w-full px-2 py-1.5 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 text-sm resize-none shadow-sm"></textarea></div>
                <div class="flex items-center"><input id="session-is-baseline" type="checkbox" class="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"><label for="session-is-baseline" class="ml-2 block text-xs font-medium text-gray-700">Marcar como Linha de Base</label></div>
                <div class="flex justify-end space-x-2"><button type="button" id="cancel-session-entry" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-medium py-1.5 px-4 rounded-md text-xs transition duration-150 ease-in-out border border-gray-300 shadow-sm">Limpar Área</button><button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-1.5 px-4 rounded-md text-xs transition duration-150 ease-in-out shadow hover:shadow-md"><i class="fas fa-save mr-1"></i> Salvar Registro</button></div>
            </form>
            <div id="program-progress-chart" class="mt-4"><h4 class="text-base font-semibold text-gray-700 mb-2">Progresso do Programa (% Acerto)</h4><div class="relative h-64 md:h-72 bg-gray-50 rounded border border-gray-200"><canvas id="progressChartCanvas" class="absolute inset-0"></canvas></div><p id="no-progress-data-message" class="hidden text-center text-sm text-gray-500 mt-4">Nenhum dado de sessão registrado para este programa.</p></div>`;
        const form = document.getElementById('add-session-form');
        const cancelBtn = document.getElementById('cancel-session-entry');
        if(form) form.addEventListener('submit', handleAddSession);
        if(cancelBtn) cancelBtn.addEventListener('click', clearSessionProgressArea);
    ...
    async function handleAddSession(e) {
        ...
        submitButton.disabled = true; submitButton.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i> Salvando...';
        try {
            ...
            } else { const result = await response.json().catch(() => ({ errors: [{ msg: "Erro ao processar resposta do servidor." }] })); alert(`Erro ao salvar sessão: ${result.errors?.[0]?.msg || 'Falha.'}`); }
        } catch (error) { alert("Erro de conexão."); }
    ...
    function renderProgressChart(programId) {
        if (!isAuthenticated || !selectedPatient || currentUser.role !== 'terapeuta') return;
        const canvasContainer = document.getElementById('program-progress-chart'), canvas = document.getElementById('progressChartCanvas'), noDataMessage = document.getElementById('no-progress-data-message'), chartArea = canvasContainer?.querySelector('.relative');
        if (!canvas || !noDataMessage || !chartArea) return;
        if (progressChartInstance) progressChartInstance.destroy();
        ...
        progressChartInstance = new Chart(ctx, {
            ...
            options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, max: 100, ticks: { color: textColor, font: { size: 10 } }, grid: { color: gridColor, drawBorder: false } }, x: { ticks: { color: textColor, font: { size: 10 } }, grid: { display: false } } }, plugins: { legend: { display: false }, tooltip: { enabled: true, backgroundColor: '#1f2937', titleColor: '#fff', bodyColor: '#fff', padding: 10, cornerRadius: 4, displayColors: true, borderColor: (tooltipItem) => { const index = tooltipItem.tooltip.dataPoints[0].dataIndex; return index < 0 || index >= programSessionData.length ? primaryColor : (programSessionData[index]?.is_baseline || programSessionData[index]?.isBaseline ? baselineColor : primaryColor); }, borderWidth: 1, callbacks: { title: function(tooltipItems) { const index = tooltipItems[0].dataIndex; if (index < 0 || index >= programSessionData.length) return ''; const session = programSessionData[index]; return `${session.is_baseline || session.isBaseline ? '[Linha de Base] ' : ''}Data: ${formatDate(session.session_date || session.date)}`; }, label: function(context) { let label = `${context.dataset.label || ''}: ${context.parsed.y}%`; const notes = programSessionData[context.dataIndex]?.notes; if (notes) label += `\nObs: ${notes.length > 50 ? notes.substring(0, 47) + '...' : notes}`; return label; }, labelColor: function(context) { const isBaseline = programSessionData[context.dataIndex]?.is_baseline || programSessionData[context.dataIndex]?.isBaseline; return { borderColor: isBaseline ? baselineColor : primaryColor, backgroundColor: isBaseline ? baselineColor : primaryColor, borderWidth: 2, borderRadius: isBaseline ? 0 : '50%' }; }, } } }, hover: { mode: 'index', intersect: false }, interaction: { mode: 'index', intersect: false }, }
        });
    }
    function clearSessionProgressArea() {
        if (progressDetailsAreaDiv && currentUser.role !== 'pai') { progressDetailsAreaDiv.innerHTML = `<div class="text-center text-gray-500 py-6"><i class="fas fa-tasks text-3xl mb-3 text-gray-400"></i><p class="text-sm">Selecione um programa atribuído na lista ao lado para registrar uma sessão ou ver o gráfico de progresso.</p></div>`; }
        selectedProgramForProgress = null;
        if (progressChartInstance) progressChartInstance.destroy();
        if (assignedProgramsListUl && currentUser.role !== 'pai') assignedProgramsListUl.querySelectorAll('.assigned-program-item.active').forEach(item => item.classList.remove('active', 'bg-indigo-100'));
    }
    ...
    async function handleSaveNotes() {
        ...
        saveNotesBtn.disabled = true; saveNotesBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-1.5"></i> Salvando...';
        try {
            ...
            if (response.ok) {
                ...
                saveNotesBtn.disabled = false; saveNotesBtn.innerHTML = '<i class="fas fa-save mr-1.5"></i> Salvar Anotações';
            }
        } catch (error) {
    ...
    function calculateOverallAverageProgress(patient) {
        ...
        return (totalScore / interventionScores.length).toFixed(1);
    }
    function calculateTotalSessions(patientsArray) {
        return patientsArray?.reduce((total, patient) => total + (patient.sessionData?.length || 0), 0) || 0;
    }
    ...
    function openConsolidatedReportModal() {
        ...
        if (currentUser.role === 'terapeuta' && selectedPatient) {
            ...
            // Popula a seção de anotações do terapeuta para impressão
            if (printTherapistNotesContent) {
                printTherapistNotesContent.textContent = selectedPatient.general_notes || 'Nenhuma anotação geral registrada.';
            }
            ...
            if(printTherapistNotesSection) printTherapistNotesSection.classList.remove('hidden');
    ...
    function prepareParentReportForPrint() {
        ...
        const patientIdElement = parentDashboardContent.querySelector('p.text-xs');
        ...
        if(printTherapistNotesSection) printTherapistNotesSection.classList.add('hidden');
        ...
        if (parentAnnotationsPrintContent && parentAnnotationsContent) {
            parentAnnotationsPrintContent.innerHTML = parentAnnotationsContent.innerHTML;
        }

        if (parentChartsGrid && parentChartsGridPrint) {
            parentChartsGridPrint.innerHTML = '';
            ...
            if (chartsToPrint.length > 0) {
                chartsToPrint.forEach(chartWrapper => {
                    const clonedChartWrapper = chartWrapper.cloneNode(true);
        ...
        if (consolidatedReportModal) consolidatedReportModal.classList.remove('hidden');
    }
    ...
    function generateProgramGradePDF(patient) {
        if (typeof jspdf === 'undefined' || typeof jspdf.jsPDF === 'undefined') { alert("Erro: PDF não disponível."); return; }
        ...
        try { const filename = `Grade_Programas_${patient.name.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.pdf`; doc.save(filename); } catch (error) { alert("Erro ao gerar PDF."); }
    }
    function generateWeeklyRecordSheetPDF(patient) {
        ...
        const totalPages = doc.internal.getNumberOfPages(); for (let i = 1; i <= totalPages; i++) { doc.setPage(i); addFooter(doc, i, margin, pageHeight, footerFontSize, totalPages.toString()); }
        try { const filename = `Folha_Registro_${patient.name.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.pdf`; doc.save(filename); } catch (error) { alert("Erro ao gerar PDF."); }
    }
    ...
    function formatDate(dateString, format = 'long') {
        ...
        try { return date.toLocaleDateString('pt-BR', options); } catch (e) { return dateString; }
    }
